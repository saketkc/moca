...
import numpy as np
import click
from progressbar import ProgressBar, Bar, Percentage, AnimatedMarker, Timer, ETA
...
def save_score(directory, phylop_wig, gerp_wig):
    ...
    scores_phylop_mean = np.mean(scores_phylop, axis=0)
    scores_gerp_mean = np.mean(scores_gerp, axis=0)
...
def mocacli(bedfile, genome_table, genome_fasta,
    ...
    bar = ProgressBar(widgets=[Percentage(), Bar(), ETA(), Timer()], max_value=4+3*5).start()
    ...
    show_progress('Read Phylop')
    phylop_wig = wigoperations.WigReader(phylop)
    show_progress('Read Gerp')
    ...
    for motif in range(1, meme_summary['total_motifs']+1):
        ...
        show_progress('Generate Random Fasta: {}'.format(motif))
